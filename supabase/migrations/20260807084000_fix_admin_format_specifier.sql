-- Fix Postgres format() : %+s n'existe pas → erreur
-- "unrecognized format() type specifier +" sur dons Kirha / niveau perso.
...
begin
  ...
  if v_kirha_delta <> 0 then
    ...
    v_log := v_log || ' kirha' || v_kirha_delta::text;
  else
  ...
  if v_payload ? 'char_level_set' then
    ...
    v_log := v_log || ' char_set=' || v_level::text;
    ...
    v_log := v_log || ' char' || trunc(v_delta)::int::text;
  end if;
