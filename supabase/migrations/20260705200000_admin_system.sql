...
create or replace function public.admin_get_logs(p_limit int default 50)
returns jsonb language plpgsql security definer set search_path = public as $$
begin
  if not public.is_staff() then raise exception 'Accès refusé'; end if;
  return coalesce((
...
begin
  ...
  return coalesce((
    select jsonb_agg(row_to_json(t))
    from (
      select le.*, p.role, p.is_banned, p.cheat_flagged
...
grant execute on function public.admin_get_logs(int) to authenticated;
