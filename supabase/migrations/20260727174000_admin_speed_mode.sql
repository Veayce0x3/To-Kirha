-- Mode vitesse admin (×5 / ×10 / ×20) sur la save joueur + résumé fiche admin
...
begin
  ...
  -- Mode vitesse admin (×5 / ×10 / ×20)
  if v_payload ? 'speed_mode' then
    if coalesce((v_payload->>'speed_mode')::boolean, false) then
      v_speed_mult := coalesce((v_payload->>'speed_multiplier')::int, 10);
      if v_speed_mult not in (5, 10, 20) then
        v_speed_mult := 10;
      end if;
      v_speed_mode := jsonb_build_object(
        'active', true,
        'multiplier', v_speed_mult,
        'popupPending', true,
        'activatedAt', v_now_ms
      );
      v_save := jsonb_set(v_save, '{speedMode}', v_speed_mode, true);
      v_log := v_log || format(' speed_on_x%s', v_speed_mult);
    else
      v_save := jsonb_set(
        v_save,
        '{speedMode}',
        jsonb_build_object('active', false, 'multiplier', 1, 'popupPending', false, 'disabledAt', v_now_ms),
        true
      );
      v_log := v_log || ' speed_off';
    end if;
  end if;
...
-- Inclure speedMode dans le résumé fiche joueur
...
begin
  ...
  return jsonb_build_object(
    ...
    'save_summary', case when v_save is null then null else jsonb_build_object(
      ...
      'speed_mode_active', coalesce((v_save->'speedMode'->>'active')::boolean, false),
      'speed_mode_multiplier', coalesce((v_save->'speedMode'->>'multiplier')::int, 1)
    ) end,
