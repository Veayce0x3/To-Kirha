-- Admin fiche joueur : resets manuels + stats de vie enrichies
-- (les migrations 20260728* n’étaient pas appliquées en prod)

create or replace function public.admin_get_player_detail(p_user_id uuid)
returns jsonb
language plpgsql
security definer
set search_path = public, auth
as $$
declare
  v_profile profiles%rowtype;
  v_save jsonb;
  v_sells int;
  v_buys int;
  v_email text;
  v_inventory jsonb;
  v_jobs jsonb;
  v_farm jsonb;
  v_combat jsonb;
  v_res_key text;
  v_res_val numeric;
  v_owned jsonb;
  v_max_season int;
  v_manual_from_history int;
  v_prestige_from_history int;
  v_discoveries jsonb;
  v_disc_total int;
  v_achievements_unlocked int;
  v_school_permanent int;
  v_school_seasonal int;
  v_journal_pages int;
  v_cookbook int;
  v_herbarium int;
  v_max_job int;
begin
  if not public.is_staff() then raise exception 'Accès refusé'; end if;
  select * into v_profile from profiles where user_id = p_user_id;
  if not found then raise exception 'Joueur introuvable'; end if;
  select lower(email) into v_email from auth.users where id = p_user_id;
  select save_data into v_save from saves where user_id = p_user_id;
  select count(*) into v_sells from market_sell_listings where seller_id = p_user_id and qty_remaining > 0;
  select count(*) into v_buys from market_buy_offers where buyer_id = p_user_id and qty_remaining > 0;

  v_inventory := '[]'::jsonb;
  begin
    if v_save is not null and jsonb_typeof(v_save->'inventory') = 'object' then
      for v_res_key, v_res_val in
        select key, (value)::numeric
        from jsonb_each_text(v_save->'inventory')
        where value ~ '^-?[0-9]+(\.[0-9]+)?$' and (value)::numeric > 0
        order by (value)::numeric desc
        limit 250
      loop
        v_inventory := v_inventory || jsonb_build_array(jsonb_build_object('id', v_res_key, 'qty', v_res_val));
      end loop;
    end if;
  exception when others then
    v_inventory := '[]'::jsonb;
  end;

  v_jobs := '{}'::jsonb;
  v_max_job := 1;
  begin
    if v_save is not null and jsonb_typeof(v_save->'jobs') = 'object' then
      for v_res_key in select jsonb_object_keys(v_save->'jobs')
      loop
        v_jobs := v_jobs || jsonb_build_object(
          v_res_key,
          coalesce((v_save->'jobs'->v_res_key->>'level')::int, 1)
        );
        v_max_job := greatest(v_max_job, coalesce((v_save->'jobs'->v_res_key->>'level')::int, 1));
      end loop;
    end if;
  exception when others then
    v_jobs := '{}'::jsonb;
    v_max_job := 1;
  end;

  v_farm := '{}'::jsonb;
  begin
    if v_save is not null and jsonb_typeof(v_save->'farmBuildingMeta') = 'object' then
      for v_res_key in select jsonb_object_keys(v_save->'farmBuildingMeta')
      loop
        v_farm := v_farm || jsonb_build_object(
          v_res_key,
          coalesce((v_save->'farmBuildingMeta'->v_res_key->>'level')::int, 1)
        );
      end loop;
    end if;
  exception when others then
    v_farm := '{}'::jsonb;
  end;

  v_combat := '[]'::jsonb;
  begin
    v_owned := coalesce(v_save->'ownedCombatItems', '[]'::jsonb);
    if v_save is not null and jsonb_typeof(v_owned) = 'array' and jsonb_array_length(v_owned) > 0 then
      select coalesce(jsonb_agg(row_data), '[]'::jsonb) into v_combat
      from (
        select jsonb_build_object(
          'ref', ref_text,
          'item_id', coalesce(inst.item_id, ref_text),
          'rarity', coalesce(inst.rarity, 'common')
        ) as row_data
        from jsonb_array_elements_text(v_owned) as ref_text
        left join lateral (
          select elem->>'itemId' as item_id, elem->>'rarity' as rarity
          from jsonb_array_elements(
            case when jsonb_typeof(coalesce(v_save->'combatItemInstances', '[]'::jsonb)) = 'array'
              then coalesce(v_save->'combatItemInstances', '[]'::jsonb)
              else '[]'::jsonb end
          ) elem
          where elem->>'instanceId' = ref_text
          limit 1
        ) inst on true
      ) sub;
    end if;
  exception when others then
    v_combat := '[]'::jsonb;
  end;

  v_max_season := 1;
  v_manual_from_history := 0;
  v_prestige_from_history := 0;
  v_discoveries := '{}'::jsonb;
  v_disc_total := 0;
  v_achievements_unlocked := 0;
  v_school_permanent := 0;
  v_school_seasonal := 0;
  v_journal_pages := 0;
  v_cookbook := 0;
  v_herbarium := 0;

  if v_save is not null then
    v_max_season := greatest(
      1,
      coalesce((v_save->>'season')::int, 1),
      coalesce((v_save->'lifetimeStats'->>'maxSeasonReached')::int, 1),
      coalesce((v_save->'lifetimeStats'->>'lastResetSeason')::int, 1),
      coalesce((v_save->'lifetimeStats'->>'seasonsCompleted')::int, 0) + 1,
      coalesce((
        select max(coalesce((elem->>'season')::int, 1))
        from jsonb_array_elements(coalesce(v_save->'seasonHistory', '[]'::jsonb)) elem
      ), 1)
    );

    begin
      select
        coalesce(count(*) filter (where coalesce(elem->>'endedBy', '') = 'manual_reset'), 0)::int,
        coalesce(count(*) filter (where coalesce(elem->>'endedBy', '') = 'prestige' or coalesce(elem->>'endedBy', '') = ''), 0)::int
      into v_manual_from_history, v_prestige_from_history
      from jsonb_array_elements(coalesce(v_save->'seasonHistory', '[]'::jsonb)) elem;
    exception when others then
      v_manual_from_history := 0;
      v_prestige_from_history := 0;
    end;

    v_discoveries := coalesce(v_save->'lifetimeStats'->'discoveries', '{}'::jsonb);
    begin
      select coalesce(sum((value)::int), 0)::int into v_disc_total
      from jsonb_each_text(v_discoveries)
      where value ~ '^[0-9]+$';
    exception when others then
      v_disc_total := 0;
    end;

    begin
      if jsonb_typeof(coalesce(v_save->'achievements'->'completed', 'null'::jsonb)) = 'array' then
        v_achievements_unlocked := coalesce(jsonb_array_length(v_save->'achievements'->'completed'), 0);
      elsif jsonb_typeof(coalesce(v_save->'quests'->'completed', 'null'::jsonb)) = 'array' then
        v_achievements_unlocked := coalesce(jsonb_array_length(v_save->'quests'->'completed'), 0);
      end if;
    exception when others then
      v_achievements_unlocked := 0;
    end;

    begin
      v_school_permanent := coalesce(jsonb_array_length(v_save->'villageSchool'->'completedPermanent'), 0);
      v_school_seasonal := coalesce(jsonb_array_length(v_save->'villageSchool'->'completedSeasonal'), 0);
    exception when others then
      v_school_permanent := 0;
      v_school_seasonal := 0;
    end;

    begin
      if jsonb_typeof(coalesce(v_save->'travelerJournal'->'unlocked', 'null'::jsonb)) = 'array' then
        v_journal_pages := coalesce(jsonb_array_length(v_save->'travelerJournal'->'unlocked'), 0);
      end if;
    exception when others then
      v_journal_pages := 0;
    end;

    begin
      v_cookbook := coalesce(jsonb_array_length(v_save->'cookbook'->'discovered'), 0);
    exception when others then
      v_cookbook := 0;
    end;

    begin
      v_herbarium := coalesce(jsonb_array_length(v_save->'herbarium'->'discovered'), 0);
    exception when others then
      v_herbarium := 0;
    end;
  end if;

  return jsonb_build_object(
    'profile', jsonb_build_object(
      'user_id', v_profile.user_id, 'display_name', v_profile.display_name,
      'role', v_profile.role, 'is_banned', v_profile.is_banned,
      'banned_at', v_profile.banned_at, 'banned_reason', v_profile.banned_reason,
      'cheat_flagged', v_profile.cheat_flagged, 'cheat_notes', v_profile.cheat_notes,
      'free_rename_used', v_profile.free_rename_used, 'email', v_email,
      'created_at', v_profile.created_at
    ),
    'name_history', coalesce((
      select jsonb_agg(row_to_json(h) order by h.created_at desc)
      from (
        select old_name, new_name, change_type, created_at
        from display_name_history
        where user_id = p_user_id
        order by created_at desc
        limit 20
      ) h
    ), '[]'::jsonb),
    'leaderboard', (select row_to_json(le) from leaderboard_entries le where le.user_id = p_user_id),
    'save_summary', case when v_save is null then null else jsonb_build_object(
      'kirha', coalesce(round(coalesce((v_save->>'kirha')::numeric, 0)), 0),
      'season', coalesce((v_save->>'season')::int, 1),
      'char_level', coalesce((v_save->'character'->>'level')::int, 1),
      'max_job_level', v_max_job,
      'nickname', v_save->'character'->>'nickname',
      'last_online', v_save->>'lastOnline',
      'save_updated_at', (select updated_at from saves where user_id = p_user_id),
      'season_started_at', v_save->>'seasonStartedAt',
      'career_confirmed', coalesce((v_save->'careerChoice'->>'confirmed')::boolean, false),
      'career_harvest', v_save->'careerChoice'->>'harvest',
      'career_farm', v_save->'careerChoice'->>'farm',
      'career_weapon', v_save->'careerChoice'->>'weaponType',
      'career_team', v_save->'careerChoice'->>'teamWeaponTypes',
      'playtime_foreground_ms', coalesce(round(coalesce((v_save->'playtime'->>'foregroundMs')::numeric, 0))::bigint, 0),
      'playtime_background_ms', coalesce(round(coalesce((v_save->'playtime'->>'backgroundMs')::numeric, 0))::bigint, 0),
      'lifetime_earned', coalesce(round(coalesce((v_save->'lifetimeStats'->>'totalEarned')::numeric, 0)), 0),
      'season_earned', coalesce(round(coalesce((v_save->'stats'->>'totalEarned')::numeric, 0)), 0),
      'lifetime_harvests', coalesce((v_save->'lifetimeStats'->>'totalHarvests')::bigint, 0)
        + coalesce((v_save->'stats'->>'totalHarvests')::bigint, 0),
      'season_harvests', coalesce((v_save->'stats'->>'totalHarvests')::bigint, 0),
      'seasons_completed', coalesce((v_save->'lifetimeStats'->>'seasonsCompleted')::int, 0),
      'season_history', coalesce(v_save->'seasonHistory', '[]'::jsonb),
      'admin_revision', coalesce((v_save->>'adminRevision')::bigint, 0),
      'game_resets', greatest(
        coalesce((v_save->'lifetimeStats'->>'gameResets')::int, 0),
        v_manual_from_history
      ),
      'resets_from_history', v_manual_from_history,
      'prestige_from_history', v_prestige_from_history,
      'last_reset_at', v_save->'lifetimeStats'->>'lastResetAt',
      'last_reset_season', (v_save->'lifetimeStats'->>'lastResetSeason')::int,
      'max_season_reached', v_max_season,
      'boss_kills_total', coalesce((v_save->'lifetimeStats'->>'bossKillsTotal')::bigint, 0),
      'dungeon_clears', coalesce((v_save->'lifetimeStats'->>'dungeonClears')::bigint, 0),
      'combat_fights', coalesce((v_save->'lifetimeStats'->>'combatFights')::bigint, 0)
        + coalesce((v_save->'stats'->>'combatFights')::bigint, 0),
      'nuggets_spent_on_seasons', coalesce((v_save->'lifetimeStats'->>'nuggetsSpentOnSeasons')::int, 0),
      'gold_nuggets', coalesce(round(coalesce((v_save->'inventory'->>'gold_nugget')::numeric, 0)), 0),
      'ancient_scrolls', coalesce(round(coalesce((v_save->'inventory'->>'ancient_scroll')::numeric, 0)), 0),
      'discoveries', v_discoveries,
      'discoveries_total', v_disc_total,
      'achievements_unlocked', v_achievements_unlocked,
      'school_permanent', v_school_permanent,
      'school_seasonal', v_school_seasonal,
      'journal_pages', v_journal_pages,
      'cookbook_recipes', v_cookbook,
      'herbarium_entries', v_herbarium,
      'prestige_kirha_pct', round(coalesce((v_save->'prestige'->>'kirhaBonus')::numeric, 0) * 100),
      'prestige_xp_pct', round(coalesce((v_save->'prestige'->>'xpBonus')::numeric, 0) * 100),
      'prestige_job_xp_pct', round(coalesce((v_save->'prestige'->>'jobXpBonus')::numeric, 0) * 100),
      'prestige_regrowth_pct', round(coalesce((v_save->'prestige'->>'regrowthSpeedBonus')::numeric, 0) * 100),
      'traveling_merchant_met', coalesce((v_save->'lifetimeStats'->>'travelingMerchantMet')::boolean, false),
      'sakura_wind_seen', coalesce((v_save->'lifetimeStats'->>'sakuraWindSeen')::boolean, false)
    ) end,
    'inventory_summary', v_inventory,
    'jobs_summary', v_jobs,
    'farm_summary', v_farm,
    'combat_items', v_combat,
    'market_sells_active', v_sells,
    'market_buys_active', v_buys,
    'reports_against', (select count(*) from player_reports where reported_user_id = p_user_id),
    'reports_by', (select count(*) from player_reports where reporter_id = p_user_id)
  );
end;
$$;

grant execute on function public.admin_get_player_detail(uuid) to authenticated;
