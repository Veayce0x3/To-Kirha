-- Admin : ajuster Kirha / inventaire / niveaux (donner ou prendre)
...
begin
  ...
  -- Inventaire : clear ids sélectionnés
  ...
  -- Inventaire : deltas { "frene": 10, "ble": -5 }
  ...
  if jsonb_typeof(v_payload->'job_level_deltas') = 'object' then
    ...
    loop
      ...
      v_jobs := jsonb_set(
        ...
        jsonb_build_object(
          'level', v_level,
          'xp', coalesce((v_jobs->v_key->>'xp')::numeric, 0)
        ),
        true
      );
    end loop;
    v_log := v_log || ' jobs_delta';
  end if;

  -- Métiers : set absolu
  if jsonb_typeof(v_payload->'job_level_sets') = 'object' then
    for v_key, v_set in
      select key, trunc(value::text::numeric)::int
      from jsonb_each(v_payload->'job_level_sets')
    loop
      if v_set < 1 then v_set := 1; end if;
      if v_set > 200 then v_set := 200; end if;
      v_jobs := jsonb_set(
        v_jobs,
        array[v_key],
        jsonb_build_object(
    ...
    v_log := v_log || ' jobs_set';
  end if;
  v_save := jsonb_set(v_save, '{jobs}', v_jobs, true);

  -- Personnage
  v_char := coalesce(v_save->'character', '{"level":1,"xp":0}'::jsonb);
  if v_payload ? 'char_level_delta' then
    v_level := coalesce((v_char->>'level')::int, 1) + trunc(coalesce((v_payload->>'char_level_delta')::numeric, 0))::int;
    if v_level < 1 then v_level := 1; end if;
    if v_level > 200 then v_level := 200; end if;
    v_char := jsonb_set(v_char, '{level}', to_jsonb(v_level), true);
    v_log := v_log || ' char_delta';
  end if;
  if v_payload ? 'char_level_set' and (v_payload->>'char_level_set') is not null then
    v_set := trunc((v_payload->>'char_level_set')::numeric)::int;
    if v_set < 1 then v_set := 1; end if;
    if v_set > 200 then v_set := 200; end if;
    v_char := jsonb_set(v_char, '{level}', to_jsonb(v_set), true);
    v_log := v_log || ' char_set';
  end if;
  v_save := jsonb_set(v_save, '{character}', v_char, true);

  -- Ferme bâtiments : deltas de niveau
