...
import { renderTravelerJournal } from './travelerJournalView.js';
...
function renderCharToolsTab(game, panel) {
  ...
  const upgradeHint = season >= (game.balance.toolSeasonUpgrade?.minSeason ?? 2)
    ...
    : '';
  panel.innerHTML = `
    <p class="view-desc char-tools-desc">Outils de récolte équipés ou en réserve. Chaque outil sert à <strong>une</strong> ressource précise (ex. hache de Séquoia → Chêne uniquement). Pour l’élevage : seau et panier peuvent être équipés ensemble.</p>
    ${upgradeHint}
