...
async function main() {
  ...
  const [
    ...
    changelog, villageBoard, villageSchool, travelerJournal,
  ] = await Promise.all([
    load('resources.json'),
    load('jobs.json'),
    load('recipes.json'),
    ...
    load('village_school.json').catch(() => ({ branches: {}, researches: {} })),
    load('traveler_journal.json').catch(() => ({ entries: {} })),
  ]);
  ...
  const game = new Game(
    ...
    weaponRoles, villageBoard, villageSchool, travelerJournal
  );
